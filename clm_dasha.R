...
run_wide_models_spline <- function(d_wide, pheno, covariates, pair){
  protein <- pair$prot
  ph <- pair$pheno
  ...
  # 2 -> 1
  formula_direct_1 <- as.formula("prot.O ~ s(pheno.F,k=3) + s(prot.F,k=3) + Age.F + BMI.F +batch.F + from.F + storage_months.F + s(ID, bs='re')")
  formula_inverse_1 <- as.formula("pheno.O ~ s(prot.F,k=3) + s(pheno.F,k=3) + Age.F + BMI.F +batch.F + from.F + storage_months.F + s(ID, bs='re')")
  
  fit_direct_1 <- gam(formula_direct_1, method = 'REML', data = wide_data)
  fit_inverse_1 <- gam(formula_inverse_1, method = 'REML', data = wide_data)
  
  p_direct_1 <- summary(fit_direct_1)$s.table["s(pheno.F)", 4]
  p_inverse_1 <- summary(fit_inverse_1)$s.table["s(prot.F)", 4]
  
  direction1 <- ifelse(p_direct_1 < 0.05 & p_inverse_1 >= 0.05, paste(ph, "→", protein),
                       ifelse(p_inverse_1 < 0.05 & p_direct_1 >= 0.05, paste(protein, "→", ph),
                              ifelse(p_direct_1 < 0.05 & p_inverse_1 < 0.05, "both directions significant", "nothing significant")))
  # 3 -> 2
  formula_direct_2 <- as.formula("prot.EL ~ s(pheno.O,k=3) + s(prot.O,k=3) +
                                 Age.O + BMI.O +batch.O + from.O + storage_months.O + s(ID, bs='re')")
  formula_inverse_2 <- as.formula("pheno.EL ~ s(prot.O,k=3) + s(pheno.O,k=3) + 
                                  Age.O + BMI.O +batch.O + from.O + storage_months.O + s(ID, bs='re')")
  
  fit_direct_2 <- gam(formula_direct_2, method = 'REML', data = wide_data)
  fit_inverse_2 <- gam(formula_inverse_2, method = 'REML', data = wide_data)
  
  p_direct_2 <- summary(fit_direct_2)$s.table["s(pheno.O)", 4]
  p_inverse_2 <- summary(fit_inverse_2)$s.table["s(prot.O)", 4]
  ...
  # 4 -> 3
  formula_direct_3 <- as.formula("prot.LL ~ s(pheno.EL,k=3) + s(prot.EL,k=3) + 
                                 Age.EL + BMI.EL +batch.EL + from.EL + storage_months.EL + s(ID, bs='re')")
  formula_inverse_3 <- as.formula("pheno.LL ~ s(prot.EL,k=3) + s(pheno.EL,k=3) + 
                                  Age.EL + BMI.EL +batch.EL + from.EL + storage_months.EL + s(ID, bs='re')")
  
  fit_direct_3 <- gam(formula_direct_3, method = 'REML', data = wide_data)
  fit_inverse_3 <- gam(formula_inverse_3, method = 'REML', data = wide_data)
  
  p_direct_3 <- summary(fit_direct_3)$s.table["s(pheno.EL)", 4]
  p_inverse_3 <- summary(fit_inverse_3)$s.table["s(prot.EL)", 4]
