...
#get the abundances
rel_abb_genus <- t(read.table('~/Desktop/P1P2P3/new_classification/nuove_considerazioni/data/Tot_Genera_Rel_ab_P1P2P3_NewClass.txt'))
...
rel_abb_species <- t(read.table('~/Desktop/P1P2P3/new_classification/nuove_considerazioni/data/Tot_Species_Rel_ab_P1P2P3_NewClass_gass.csv'))
...
df_lab <- read.csv('~/Desktop/P1P2P3/new_classification/nuove_considerazioni/data/Metadata_Qbit_libray_DNA_P1P2P3.csv',sep = ';')[,-c(1,5)]
colnames(df_lab)[1] <- 'Code'
df_lab$Code <- sapply(df_lab$Code,function(x){paste('X',x,sep = '')})

df_reads <- read.csv('~/Desktop/P1P2P3/new_classification/nuove_considerazioni/data/Quality_trimming_resume_P1P2P3.csv',sep = ';')
df_reads$Code <- sapply(df_reads$Code,function(stringa){paste0('X',stringa)})

df_lab <- merge(df_lab,df_reads, by = 'Code')
correction <- commandArgs(trailingOnly = TRUE) #correct the pcoa or not
correction <- ifelse(correction == 'T', TRUE, ifelse(correction == 'F', FALSE, stop('Enter a valid parameter')))

#get the questionnaire
session <- ssh_connect("nome.cognome@IP",passwd = '')
file_path <- '/home/res-fellows/nome.cognome/cleaned_data/cleaned_questionnaire.csv'
data <- ssh_exec_internal(session, paste("cat", file_path))
quest <- read.csv(text = rawToChar(data$stdout))
...
for(i in 1:5){
  ...
  test3 <- cor.test(df_lab_pcoa_gen[[name_pc]], as.numeric(df_lab_pcoa_gen$Input.Read.Pairs), method = 'spearman',exact = F)
  results_df <- rbind(results_df, data.frame(
    PCoA = name_pc,
    Comparison = 'Input_Reads vs genus PCoA',
    Estimate = test3$estimate,
    P_Value = test3$p.value
  ))
  test4 <- cor.test(df_lab_pcoa_gen[[name_pc]], as.numeric(df_lab_pcoa_gen$Survived.Reads.Paired), method = 'spearman',exact = F)
  results_df <- rbind(results_df, data.frame(
    PCoA = name_pc,
    Comparison = 'Surv_Reads vs genus PCoA',
    Estimate = test4$estimate,
    P_Value = test4$p.value
  ))
  test5 <- cor.test(df_lab_pcoa_spe[[name_pc]], as.numeric(df_lab_pcoa_spe$Qubit_DNA), method = 'spearman',exact = F)
  results_df <- rbind(results_df, data.frame(
    PCoA = name_pc,
  ...
  test6 <- cor.test(df_lab_pcoa_spe[[name_pc]], as.numeric(df_lab_pcoa_spe$Qubit_Library), method = 'spearman',exact = F)
  results_df <- rbind(results_df, data.frame(
    PCoA = name_pc,
  ...
  test7 <- cor.test(df_lab_pcoa_spe[[name_pc]], as.numeric(df_lab_pcoa_spe$Input.Read.Pairs), method = 'spearman',exact = F)
  results_df <- rbind(results_df, data.frame(
    PCoA = name_pc,
    Comparison = 'Input_Reads vs species PCoA',
    Estimate = test7$estimate,
    P_Value = test7$p.value
  ))
  test8 <- cor.test(df_lab_pcoa_spe[[name_pc]], as.numeric(df_lab_pcoa_spe$Survived.Reads.Paired), method = 'spearman',exact = F)
  results_df <- rbind(results_df, data.frame(
    PCoA = name_pc,
    Comparison = 'Surv_Reads vs species PCoA',
    Estimate = test8$estimate,
    P_Value = test8$p.value
  ))
}
...
df_lab <- df_lab[,c(1:3,5,6)]
colnames(df_lab)[4:5] <- c('Input_Reads','Surv_Reads')
df_lab$Input_Reads <- as.numeric(df_lab$Input_Reads)
df_lab$Surv_Reads <- as.numeric(df_lab$Surv_Reads)

#create a function that correct linearly the PCoAs from pcoa_simple using the results calculated above
...
independence_test <- function(col,choice,visit = 1,plot = T,save_plot = F,print_plot = T){
  ...
  for(i in 1:5){ #I got i varying across 1,2,3,4,5
    ...
    } #prendo le colonne relative alla i-esima componente e al j-esimo valore di values
    group <- NULL 
    for(val in values){ 
  ...
  if(plot){ #check if the user wanna plot the pcoa
    ...
    if(save_plot){ #check if the user wanna save the plot
      ...
      name_file <- paste('independence_tests_',col,'_visit_',visit_name,'.jpg',sep = '') #name of the file
      path_folder <- paste('~/Desktop/P1P2P3/new_classification/nuove_considerazioni/',
                           choice,'/test_indipendenza/',sep = '') #path of the folder
...
create_plot_independence <- function(to_check, choice, visit = 1, save_plot = F) {
  ...
  if (save_plot) {
    ...
    name_file <- paste('complete_independence_tests_', choice, '_visit_', visit_name, '.jpg', sep = '')  # Name of the file
    path_folder <- paste('~/Desktop/P1P2P3/new_classification/nuove_considerazioni/', choice, '/test_indipendenza/', sep = '')  # Path of the folder
    path_folder <- ifelse(correction,paste0(path_folder,'corrected/'),
                          paste0(path_folder,'not_corrected/'))
    complete_path <- paste(path_folder, name_file, sep = '')  # Complete path of the file
...
make_analysis <- function(to_check,choice = c('genus','species'),visit = 1,save_results = T,save_plots = F){
  ...
  for(cl in choice){
    ...
    if(save_results){
      ...
      path_folder <- paste('~/Desktop/P1P2P3/new_classification/nuove_considerazioni/',
                           cl,'/test_indipendenza/',sep = '') #path of the folder
      path_folder <- ifelse(correction,paste0(path_folder,'corrected/'),
                            paste0(path_folder,'not_corrected/'))
      complete_path <- paste(path_folder,name_file, sep = '') #complete path of the file
...
all_results_genus <- do.call(rbind, list_of_results_genus)
...
all_results_genus_long$color <- sapply(all_results_genus_long$value, function(p) {
  if (p < 0.01) {
    return('p_value < 0.01')
  } else if ((p < 0.05) & (p >= 0.01)) {
    return('0.01 < p_value < 0.05')
...
genus_grid <- ggplot(all_results_genus_long, aes(PCoA, feature, fill = color)) + geom_tile(color = "black") + 
  ...
  scale_fill_manual(values = c('p_value < 0.01' = 'darkgreen', '0.01 < p_value < 0.05' = "#aefcb9", 'p_value > 0.05' = "white"),  
                    name = "Significance Level") +
  scale_x_discrete(expand = c(0,0)) + scale_y_discrete(expand = c(0,0)) +
  xlab('PCoA') +  
  ylab('Feature') + 
  facet_wrap(~ Visit_number, nrow = 1) 
print(genus_grid)
...
path_folder <- '~/Desktop/P1P2P3/new_classification/nuove_considerazioni/genus/test_indipendenza/'
...
path_folder <- '~/Desktop/P1P2P3/new_classification/nuove_considerazioni/species/test_indipendenza/'
path_folder <- ifelse(correction, paste0(path_folder,'corrected/'),paste0(path_folder,'not_corrected/'))
...
for(choice in c('genus','species')){
  ...
  for(vis in 1:4){
    path <- file.path('~/Desktop/P1P2P3/new_classification/nuove_considerazioni',
                      choice,
                      'test_indipendenza/corrected',
...
write.csv(significant_result, '~/Desktop/P1P2P3/new_classification/nuove_considerazioni/data/significant_variable_independence_test.csv')

final_variables <- union(final_variables_genus,final_variables_species)

writeLines(final_variables,'~/Desktop/P1P2P3/new_classification/nuove_considerazioni/data/independence_significant_variables.txt')
...
ggsave('~/Desktop/P1P2P3/new_classification/nuove_considerazioni/comparison_genus_correct_not_correct.jpg', grid.arrange(g1,g2))
ggsave('~/Desktop/P1P2P3/new_classification/nuove_considerazioni/comparison_species_correct_not_correct.jpg', grid.arrange(s1,s2))


write.csv(subset(results_df,P_Value < 0.05),'~/Desktop/P1P2P3/new_classification/nuove_considerazioni/results_correlations_pcoa_technical_variables.csv')
