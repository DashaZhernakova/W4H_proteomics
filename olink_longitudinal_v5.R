...
h1 <- pheatmap(limma_heatmap, color = c(my_colors[3], "white", my_colors[2]), legend = F, cluster_cols = F)
...
prot = 'PROK1'
pdf(paste0(out_basedir, "plots/selected_prot_trajs.pdf"), width = 3, height = 3)
plot_boxplot_with_traj(d_wide_adj_covar, 'PROK1')
plot_boxplot_with_traj(d_wide_adj_covar, 'FABP4')
plot_boxplot_with_traj(d_wide_adj_covar, 'REN')
plot_boxplot_with_traj(d_wide_adj_covar, 'TNFSF10')
dev.off()

# TNF
p1 <- plot_traj_many_prots2(all_prots_traj, prots = c("FAS", "FASLG", "TNFSF10", "TNFSF11", "TNFSF12", "TNFRSF10C", "TNFRSF13B"),colored = T, phases = T)
p2 <- plot_boxplot_with_traj(d_wide_adj_covar, 'FAS')
p3 <- plot_boxplot_with_traj(d_wide_adj_covar, 'FASLG')
p4 <- plot_boxplot_with_traj(d_wide_adj_covar, 'TNFSF10')
p5 <- plot_boxplot_with_traj(d_wide_adj_covar, 'TNFSF11')
p6 <- plot_boxplot_with_traj(d_wide_adj_covar, 'TNFSF12')
p7 <- plot_boxplot_with_traj(d_wide_adj_covar, 'TNFRSF10C')
p8 <- plot_boxplot_with_traj(d_wide_adj_covar, 'TNFRSF13B')

pdf(paste0(out_basedir, "plots/TNF_plots.pdf"), width = 10, height = 10, useDingbats = F)
design <- "
  112
  113
  456
  78#
"
p1+p2+p3+p4+p5+p6+p7+p8 + plot_layout(design = design)
dev.off()
...
################################################################################
# Protein vs phenotype  lm per phase
################################################################################

lm_per_tp <- data.frame(matrix(nrow = length(all_prots) * length(all_hormones) * length(all_phases), ncol = 5))
colnames(lm_per_tp) <- c("pheno", "prot", "phase", "estimate", "pval")

results_list <- vector("list", length(all_hormones) * length(all_prots))
idx <- 1

for (ph in all_hormones) {
  for (prot in all_prots){
    results_list[[idx]] <- lm_per_tp_pheno_prot_adj_covar(d_wide, pheno, prot, ph, covariates, scale = T)
    idx <- idx + 1
  }
}

lm_per_tp <- do.call(rbind, results_list)

lm_per_tp <- na.omit(lm_per_tp) %>%
  mutate(across(-c(pheno, prot, phase), as.numeric)) %>%
  group_by(phase) %>%
  mutate(BH_pval <- p.adjust(pval, method = 'BH')) %>%
  ungroup()
  
lm_per_tp <- lm_per_tp[order(lm_per_tp$pval),]
write.table(lm_per_tp, file = paste0(out_basedir, "prot_vs_pheno_per_tp.txt"), quote = F, sep = "\t", col.names = NA, row.names = T)
...
#network_data <- network_data[abs(network_data$estimate) > 0.15,]

# only take proteins that have at least 2 phenotypes/hormones associated
#prot_counts <- table(c(network_data$prot, network_data$pheno))
#prot_sel <- names(prot_counts[prot_counts > 1])
#network_data <- network_data[network_data$prot %in% prot_sel,]
